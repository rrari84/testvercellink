...
export class PasskeyService {
  ...
  constructor(contracts?: ContractAddresses) {
    ...
    // Use provided contract addresses from your deployment
...
// Add this test to see if the contract is working at all
async testContractExists(): Promise<void> {
  try {
    console.log('🧪 Testing if trading contract exists and responds...');
    ...
    // Try to call a non-existent function to see the contract's response
    const tx = new TransactionBuilder(dummyAccount, {
      fee: BASE_FEE,
      networkPassphrase: this.networkPassphrase,
    })
      .addOperation(contract.call('nonexistent_function_test_12345'))
      .setTimeout(30)
      .build();
    ...
    if (rpc.Api.isSimulationError(result)) {
      console.log('Contract response to invalid function:', result.error);
      
      // If we get a "non-existent function" error, the contract exists
      if (result.error.includes('non-existent contract function')) {
        console.log('✅ Contract exists and is responding');
        console.log('❌ But it has no trading functions');
        console.log('🤔 This suggests the deployed contract is not a trading contract');
      }
    }
  ...
  // Add this method to PasskeyService.ts
async debugTradingContractFunctions(): Promise<void> {
  console.log('🔍 Debugging trading contract functions...');
  ...
  // Common trading function names to try
  const functionNames = [
    'open_long', 'open_short', 'long', 'short', 'buy', 'sell',
    'open_position', 'place_order', 'submit_order', 'create_order',
    'trade', 'execute_trade', 'add_position', 'new_position',
    'open', 'close', 'enter_position', 'exit_position'
  ];

  const tradingContract = new Contract(this.contracts.trading);
  ...
  for (const funcName of functionNames) {
    try {
      const tx = new TransactionBuilder(dummyAccount, {
        ...
        .addOperation(tradingContract.call(funcName))
        .setTimeout(30)
        .build();
      ...
      if (rpc.Api.isSimulationSuccess(result)) {
        console.log(`✅ ${funcName}() - EXISTS and works without parameters`);
      } else if (rpc.Api.isSimulationError(result)) {
        const error = result.error;
        if (error.includes('function not found') || error.includes('non-existent contract function')) {
          console.log(`❌ ${funcName}() - DOES NOT EXIST`);
        } else {
          console.log(`⚠️  ${funcName}() - EXISTS but needs parameters:`, error);
        }
      }
    } catch (error) {
...
async executeTrade(
  ...
  // Close a position using the submit function
async closePositionRequest(positionId: number): Promise<{ success: boolean; error?: string }> {
  try {
    const authResult = await this.authenticate();
    if (!authResult.success) {
      return { success: false, error: 'Authentication required' };
    ...
    const closeRequest = nativeToScVal({
      action: { tag: 'Close' },
      data: null,
      position: positionId
    });
    ...
    const transaction = new TransactionBuilder(account, {
      ...
      .addOperation(
        contract.call(
          'submit',
          nativeToScVal(Address.fromString(userKeypair.publicKey()), {type: 'address'}),
          nativeToScVal([closeRequest], {type: 'vec'})
        )
      )
      .setTimeout(30)
      .build();
  ...
  async getPrices(): Promise<{ success: boolean; prices?: any; error?: string }> {
    try {
      ...
      return {
        success: true,
        prices: {
          KALE: { price: 1.0, timestamp: Date.now() }, // From deployment: "kalePrice": "1.0"
          XLM: { price: 0.50, timestamp: Date.now() }   // From deployment: "xlmPrice": "0.50"
        }
      };
...
// Try to see if the user needs to be registered first
// Add this to PasskeyService.ts
...
async checkContractInitialization(): Promise<void> {
  try {
    ...
    // Check if contract has an owner (indicates initialization)
...
// 2. Check if XLM market is configured
async checkMarketConfiguration(): Promise<void> {
  try {
    ...
    // Try to get market data for XLM
    const xlmAsset = nativeToScVal({ tag: 'Other', values: ['XLM'] }, { type: 'enum' });
    
    // This would require a function to get market config - let's try a different approach
    // Check if we can call set_market to see if it's configured
...
// 3. Use the correct submit function instead of create_position
async executeTradeViaSubmit(
  ...
  try {
    ...
    const entryPrice = BigInt(Math.floor(0.50 * 10**7)); // XLM price
    
    // Create a request to open a position using the submit function
    // Based on the contract interface, we need to create a Request struct
    const openPositionRequest = nativeToScVal({
      action: nativeToScVal(1, { type: 'u32' }), // Fill = 1 (closest to opening position)
      data: nativeToScVal(entryPrice, { type: 'i128' }), // Entry price as data
      position: nativeToScVal(0, { type: 'u32' }) // New position, so ID 0
    });
    ...
    if (rpc.Api.isSimulationError(simulationResult)) {
      console.log('❌ Submit simulation failed:', simulationResult.error);
      return { 
        success: false, 
...
// 4. Enhanced debug function to understand the contract state
async debugContractState(): Promise<void> {
  try {
    ...
    // Try to get user positions (if function exists)
    const dummyAccount = new Account(
      'GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF',
      '0'
    );
    ...
    // Test submit with an empty request array to see what happens
    try {
      const emptySubmitTx = new TransactionBuilder(dummyAccount, {
        ...
        .addOperation(
          contract.call(
            'submit',
            nativeToScVal(Address.fromString(userKeypair.publicKey()), {type: 'address'}),
            nativeToScVal([], {type: 'vec'}) // Empty requests array
          )
        )
        .setTimeout(30)
        .build();
  ...
  // Discover available contract functions
  private async discoverContractFunctions(): Promise<void> {
    ...
    // Test trading contract functions
    const tradingFunctions = [
      'open_long', 'open_short', 'close_position', 'submit_order',
      'get_positions', 'get_user_positions', 'place_order', 'long', 'short',
      'buy', 'sell', 'open_position'
    ];
    ...
    for (const func of tradingFunctions) {
      try {
        const tx = new TransactionBuilder(dummyAccount, {
          ...
          .addOperation(tradingContract.call(func))
          .setTimeout(30)
          .build();
        ...
        if (rpc.Api.isSimulationSuccess(result)) {
          console.log(`✅ Trading.${func}() - Available`);
        } else if (rpc.Api.isSimulationError(result)) {
          const error = result.error;
          if (!error.includes('function not found') && !error.includes('unknown function')) {
            console.log(`⚠️  Trading.${func}() - Exists but needs parameters`);
          }
        }
      } catch (error) {
